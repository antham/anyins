;;; anyins-steps.el --- Step for anyins integration tests
...
(Then "^I should see in buffer$"
      (lambda (expected-content)
        (let ((actual-content (buffer-string))
          ...
          (cl-assert (s-equals? expected-content actual-content) nil message expected-content
                     actual-content))))
...
(provide 'anyins-steps)

;;; anyins-steps.el ends here
